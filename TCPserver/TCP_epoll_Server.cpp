...
int main()
{
    int sockfd, nfds, sendfd, cfd, ret;
    int on = 1;
    ...
    while(true)
    {
		...
		if ( buffer[ret-2] == '#')
		{
			...
			strncpy(strb, buffer + 6, ret-8);
			std::cout << "字符串a:" << stra << "  字符串b:" << strb << std::endl;
		
			/*----------------------MYSQL MODULE----------------*/
	        	char s1[200]="INSERT INTO pgdemo(ID,stime) VALUES (\"";
  			strcat(s1,stra);
  			strcat(s1,"\",");
  			strcat(s1,strb);
  			strcat(s1,")");
			...
			mysqlpp::Connection conn(false);
                	if (conn.connect("L298N", "localhost","root", "l0098122")) {
                		mysqlpp::Query query = conn.query(s1);
                		if (mysqlpp::StoreQueryResult res = query.store()) 
				{ std::cout << "Insert OK!" << std::endl;}
				else { std::cout << "Insert OK!" << std::endl;}
			}
			/*------------------------MYSQL MODULE END------------*/
			...
			bzero(&buffer, sizeof(buffer));
                	bcopy("OK\n", buffer, sizeof("OK\n"));
                	sendfd = send(events[i].data.fd, buffer, sizeof(buffer), 0);			
			/*----------------------SEND BACK INFO END----------------*/
